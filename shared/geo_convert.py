#!/usr/bin/env python3
"""Convert v2ray geosite/geoip protobuf .dat files to sing-box rule-set JSON.

Only emits categories supplied as arguments.
...
def _iter_fields(
    buf: memoryview,
) -> Iterator[tuple[int, int | None, memoryview | None]]:
    i = 0
    end = len(buf)
    while i < end:
...
def _parse_geosite_entry(buf: memoryview) -> tuple[str, list]:
    code = ""
    domains = []
    for field, _ival, sub in _iter_fields(buf):
        if field == 1:
...
def main() -> int:
    ...
    for entry in out_dir.iterdir():
        fname = entry.name
        if (
            fname.startswith(("geosite-", "geoip-"))
            and fname.endswith(".json")
            and fname not in expected
        ):
            entry.unlink()
    ...
    if args.geosite:
        ...
        for cat in args.geosite:
            ...
            path = out_dir / f"geosite-{cat_lc}.json"
    ...
    if args.geoip:
        ...
        for code in args.geoip:
            ...
            path = out_dir / f"geoip-{code_lc}.json"
            with path.open("w") as f:
                json.dump(ruleset, f, indent=2)
            print(f"wrote {path}")
